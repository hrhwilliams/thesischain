...
const { data: backendVersion } = useQuery({
  queryKey: ["version"],
  queryFn: async () => {
    const response = await request<{ version: string; commit: string }>(
      "/version",
      "GET",
    );
...
<template>
  <div class="device-list">
    ...
    <ul>
      <li v-for="device in devices">
        <div v-if="device.device_id === getDeviceId()">
          <code>{{ device.device_id }}</code> (this device)
        </div>
        <div v-else>
  ...
  <code v-if="backendVersion"
    >{{ backendVersion.version }} ({{ backendVersion.commit }})</code
  >
</template>
